...
describe('LeaveRepository Test Suite', () => {
  ...
  it('should create new leave request successfully', async () => {
    ...
    const spyCreate = jest.spyOn(Leave, 'create').mockImplementation((async (payload) => {
      return { ...mockLeave, ...payload } ;
    }) );

    const result = await leaveRepository.create(mockLeave );
  ...
  it('should identify overlapping leave applications correctly', async () => {
    const spyFindOne = jest.spyOn(Leave, 'findOne').mockResolvedValue({ _id: 'leave_123' } );
