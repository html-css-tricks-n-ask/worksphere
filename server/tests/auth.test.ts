...
describe('AuthService Integration & Registration Suite', () => {
  ...
  describe('registerCompany', () => {
    it('should throw an error if company email already exists', async () => {
      const findCompanySpy = jest.spyOn(companyRepository, 'findByEmail').mockResolvedValue({ id: 'comp_1' } as any);
    ...
    it('should throw an error if admin email already exists', async () => {
      ...
      const findUserSpy = jest.spyOn(userRepository, 'findByEmail').mockResolvedValue({ id: 'usr_1' } as any);
      const userCreateSpy = jest.spyOn(userRepository, 'create');
    ...
    it('should successfully register company and admin and trigger verify email link', async () => {
      ...
      const companyCreateSpy = jest.spyOn(companyRepository, 'create').mockResolvedValue(mockCompany as any);
      const userCreateSpy = jest.spyOn(userRepository, 'create').mockResolvedValue(mockAdmin as any);
      const companyUpdateSpy = jest.spyOn(companyRepository, 'update').mockResolvedValue(mockCompany as any);
      const emailSpy = jest.spyOn(emailService, 'sendVerificationEmail').mockResolvedValue(true as any);
