...
describe('AttendanceRepository Test Suite', () => {
  ...
  it('should create check-in attendance record successfully', async () => {
    ...
    const spyCreate = jest.spyOn(Attendance, 'create').mockImplementation((async (payload) => {
      return { ...mockAttendance, ...payload } ;
    }) );
  ...
  it('should find employee today log', async () => {
    ...
    const spyFindOne = jest.spyOn(Attendance, 'findOne').mockResolvedValue(mockAttendance );

    const result = await attendanceRepository.findTodayLog('emp_123', 'comp_123');

    expect(_optionalChain([result, 'optionalAccess', _ => _._id])).toBe('att_123');
