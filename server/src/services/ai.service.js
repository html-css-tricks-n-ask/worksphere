 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { logger } from '../config/logger.js';
  ...
  async chat(companyId, query) {
    ...
    if (apiKey && apiKey !== 'dummy' && !apiKey.includes('your_')) {
      try {
        ...
        logger.error(`OpenAI chat request failed: ${(err ).message}`);
      }
    }
    ...
    if (lowerQuery.includes('leave') || lowerQuery.includes('vacation')) {
      ...
      const names = activeLeaves.map((l) => `${_optionalChain([l, 'access', _ => _.employeeId, 'optionalAccess', _2 => _2.firstName])} ${_optionalChain([l, 'access', _3 => _3.employeeId, 'optionalAccess', _4 => _4.lastName])} (${l.leaveType})`).join(', ');
    ...
    if (lowerQuery.includes('attendance') || lowerQuery.includes('present') || lowerQuery.includes('check')) {
      ...
      const names = checkins.map((c) => `${_optionalChain([c, 'access', _5 => _5.employeeId, 'optionalAccess', _6 => _6.firstName])} ${_optionalChain([c, 'access', _7 => _7.employeeId, 'optionalAccess', _8 => _8.lastName])}`).join(', ');
