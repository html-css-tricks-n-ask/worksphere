...
export const createShift = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const shift = await shiftRepository.create(parsed as any);
...
export const getShiftById = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
...
export const updateShift = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const shift = await shiftRepository.update(req.params.id, parsed as any);
  if (!shift) {
    throw new ApiError(404, 'Shift not found.');
  }
