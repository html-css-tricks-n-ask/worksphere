...
export const updateHoliday = asyncHandler(async (req, res) => {
  ...
  const holiday = await holidayRepository.update(req.params.id, parsed );
  if (!holiday) {
    throw new ApiError(404, 'Holiday not found.');
  }
...
export const bulkCreateHolidays = asyncHandler(async (req, res) => {
  const companyId = req.user.companyId.toString();
  const items = req.body.holidays;
  ...
  for (const item of items) {
    try {
      ...
      errors.push({ name: item.name, date: item.date, error: (err ).message });
    }
  }
